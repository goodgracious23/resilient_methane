...
sampling_depths = all_years_ch4 %>%
  ...
  summarize(sampling_depth = mean(depth, na.rm = TRUE)) %>%
  ungroup() %>%
...
sum_ch4mass = combined_vol_methane %>%
  group_by(lake, date) %>%
  summarize(total_ch4mass = sum(methane_mass)) %>%
  ungroup() %>%
...
oxic_mass = combined_mass %>% filter(!depth > Anoxia) %>%
  group_by(lake, year, doy) %>%
  summarize(oxic_mass = sum(methane_mass, na.rm = TRUE)) %>%
  ungroup()

# ggplot(oxic_mass, aes(x = doy, y = oxic_mass, fill = year, color = year)) +
#   geom_point(size = 2) +
...
light_mass = combined_mass %>% filter(!depth > Light) %>%
  group_by(lake, year, doy) %>%
  summarize(light_mass = sum(methane_mass, na.rm = TRUE)) %>%
  ungroup()

# ggplot(light_mass, aes(x = doy, y = light_mass, fill = year, color = year)) +
#   geom_point(size = 2) +
#   # scale_y_continuous(transform = "log10", labels = label_number()) +
#   # stat_smooth(method = "gam", formula = y ~ s(x, bs = "cs", k = 3)) +
#   stat_smooth(method = "glm") +
